...
static void pio_data_irq_func(void)
{
    ...
    pio_sm_drain_tx_fifo(pio_cpufifo, sm_cpufifo);
    update_status_register();
    pio_cpufifo->txf[sm_cpufifo] = datastatus_reg.status;
...
void readdata_program_init(PIO pio, uint sm, uint offset)
{
    ...
    pio_set_irqn_source_enabled(pio, irq_index, pis_interrupt0, true);                                           // Set pio to tell us when irq0 is raised
...
void writedata_program_init(PIO pio, uint sm, uint offset)
{
    static const uint base_data_pin = 2;
    static const uint cs_pin = 10;
    ...
    pio_sm_init(pio, sm_writedata, offset_writedata, &c_writedata);
...
void init_cpu_fifo()
{
    ...
    writedata_program_init(pio_cpufifo, sm_readdata, offset_writedata);
    ...
    while (true)
    {
        ...
        if (!pio_sm_is_tx_fifo_full(pio_cpufifo, sm_readdata) && uart_is_readable(uart0)) // UART RX -> PIO TX
        {
